...
def login_view():
    ...
    with st.form(key="login_form"):
        ...
        submit_button = st.form_submit_button("サインイン", type="primary")
...
def signup_view():
    ...
    st.header("新規アカウント登録")
    st.warning(
        "- **アカウントを作成**ボタンを押すとそのメールアドレスにアカウントユーザー認証メールが送信されます。  \
    ...
    with st.form(key="signup_form"):
        ...
        submit_button = st.form_submit_button("アカウントを作成", type="primary")
        ...
        if submit_button:
            ...
            if response.user and not response.session:
                st.success(
                    f"新規アカウント登録が完了しました！**{email}**宛に確認メールを送信しました。メール内のリンクをクリックしてアカウントを有効にしてください。"
                )
                ...
                try:
                    ...
                    admin_users_response = (
                        supabase_service.table("user_roles")
                        .select("email")
                        .eq("role", "admin")
                        .execute()
                    )
                    if admin_users_response.data:
                        admin_emails = [
                            user["email"] for user in admin_users_response.data
                        ]
